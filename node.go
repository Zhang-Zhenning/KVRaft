...
func (rf *Raft) SyncLogLoop(server int) {
	...
	for !rf.IsKilled {
		...
		// only leader can sync logs to other nodes!
		if isLeader {
...
// kill the node, this may cause error as other nodes may still dial to this killed node (the listener is already shutdown)
func (rf *Raft) HandleKillRequest(_ *struct{}, _ *struct{}) error {
	rf.mu.Lock()
	defer rf.mu.Unlock()
...
// shutdown listener and rpc server
func (rf *Raft) ShutdownServer() {
	rf.mu.Lock()
	defer rf.mu.Unlock()
